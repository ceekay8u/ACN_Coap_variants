    ...
    if (block_opt) { /* handle Block2 */
      ...
      if(COAP_OPT_BLOCK_MORE(block_opt)) {
        ...
        if ( pdu ) {
          ...
          printf("before : message handler");
          tid = coap_send(session, pdu);
          printf("after : message handler");
...
int main(int argc, char **argv) {
  ...
  coap_log(LOG_DEBUG, "sending request:\n");
  //coap_log(LOG_DEBUG, "before coap_show_pdu\n");
  //if (coap_get_log_level() < LOG_DEBUG)
  
  //coap_log(LOG_DEBUG, "***: new outgoing session\n");


  coap_show_pdu(LOG_INFO, pdu);

  //coap_log(LOG_DEBUG, "before coap_send\n");
  coap_send(session, pdu);
  //coap_log(LOG_DEBUG, "after coap_send\n");

  
  //coap_send(session, pdu);
  //fprintf("after client");
  ...
  while (!quit && !(ready && !doing_getting_block && coap_can_exit(ctx)) ) {
    //while (!(ready && !doing_getting_block && coap_can_exit(ctx)) ) {
