...
//added code global vairabled updated_timeout and prev_retransmit_cnt


uint16_t updated_timeout;
int if_retransmit=0;
...
coap_send(coap_session_t *session, coap_pdu_t *pdu) {
 ...
 /* if(node->retransmit_cnt==0)
  {
    prev_retransmit_cnt=0;
  }
...
*/
 ...
 /*
  ...
  /*if(prev_retransmit_cnt==0)*/
  if(if_retransmit==0)
  {
      node->timeout = max(coap_calc_timeout(session, r),(updated_timeout/2));
      updated_timeout=updated_timeout/2;
  }


  else
  {
    node->timeout=updated_timeout;
  }
...
coap_retransmit(coap_context_t *context, coap_queue_t *node) {
  ...
  if (node->retransmit_cnt < node->session->max_retransmit) {
   ...
   node->retransmit_cnt++;
    ...
    if(node->retransmit_cnt==1)
      updated_timeout=node->timeout;
     

    else if(node->retransmit_cnt==2)
      updated_timeout=node->timeout<<1;

    else
      updated_timeout=updated_timeout<<1;
